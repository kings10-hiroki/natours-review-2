...
var displayMap = function displayMap(locations) {
  ...
  var map = new mapboxgl.Map({
    container: 'map',
    style: 'mapbox://styles/8hiropon/ckfvu4ror3ad319n98kvu72r6' // center: [-118.6919161, 34.0207305],
    // zoom: 7,
    // interactive: false
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 4:
            ...
            if (res.data.status === 'success') {
              (0, _alerts.showAlert)('success', "Changed ".concat(type, " data in successfully"));
              window.setTimeout(function () {
...
},{"axios":"../../node_modules/axios/index.js","./alerts":"alerts.js"}],"imageLoader.js":[function(require,module,exports) {
"use strict";
...
var previewFile = function previewFile(el, file) {
  // FileReaderオブジェクトを作成
  var reader = new FileReader(); // ファイルが読み込まれたときに実行する

  reader.onload = function (e) {
    // 画像のURLはevent.target.resultで呼び出せる
    var imageUrl = e.target.result; // 画像のURLをimg要素にセット
...
var savePasswordBtn = document.querySelector('.btn--save');
var userPhoto = document.getElementById('user-photo');
var fileInput = document.querySelector('.form__upload-image');
var bookBtn = document.getElementById('book-tour');

if (mapBox) {
  var locations = JSON.parse(mapBox.dataset.locations); // JSON.stringifyは、JavaScriptオブジェクトをJSONテキストに変換し、そのJSONテキストを文字列に格納します。
...
if (userPhoto) {
  ...
  var handleFileSelect = function handleFileSelect() {
    var files = fileInput.files; // 複数枚のアップロードに対応

    for (var i = 0; i < files.length; i++) {
      (0, _imageLoader.previewFile)(userPhoto, files[i]);
    }
  };
...
if (logoutBtn) {
  logoutBtn.addEventListener('click', _authentication.logout);
}
